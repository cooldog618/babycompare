...
import { getComparePriceState, type CompareItem, type LowestPriceSummary } from '../lib/compare';
...
export function CompareTable({ items, onRemove, summary }: { items: CompareItem[]; onRemove: (id: string) => void; summary: LowestPriceSummary }) {
  return (
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.key}>
              ...
              {items.map((item) => {
                const priceState = getComparePriceState(item, summary);
                return (
                  <td key={`${item.id}-${row.key}`} className="border-b border-slate-200 p-3 text-sm text-slate-800 align-top">
                    {row.key === 'price' && (
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-rose-600">{formatPrice(item.price)}</span>
